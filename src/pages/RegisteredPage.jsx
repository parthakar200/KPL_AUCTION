...
// // ── Player Avatar ────────────────────────────────────────────
// function PlayerAvatar({ photo, name, size = 44, color }) {
//   return (
//     <div style={{ width:size, height:size, borderRadius:'50%', overflow:'hidden', flexShrink:0, border:`2px solid ${color || 'var(--border)'}`, background:'var(--bg-card2)', display:'flex', alignItems:'center', justifyContent:'center' }}>
//       {photo
//         ? <img src={photo} alt={name} style={{ width:'100%', height:'100%', objectFit:'cover' }} />
//         : <span style={{ fontSize: size * 0.4, userSelect:'none' }}>
...
//           style={{ width:56, height:56, borderRadius:'50%', cursor:'pointer', border: photo ? '2px solid var(--accent)' : '2px dashed var(--border)', background: photo ? 'transparent' : 'var(--bg-card)', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', flexShrink:0, transition:'all 0.2s' }}>
//           {photo
...
//         </div>
//         <div>
//           <div style={{ fontSize:11, fontWeight:700, color:'var(--text-dim)', marginBottom:4 }}>Player Photo</div>
//           <button onClick={() => photoInputRef.current?.click()}
//             style={{ fontSize:10, padding:'4px 10px', background:'var(--bg-card)', border:'1px solid var(--border)', color:'var(--text-muted)', borderRadius:6, cursor:'pointer', fontFamily:'var(--font-body)', fontWeight:600 }}>
//             {photo ? '🔄 Change Photo' : '📷 Upload Photo'}
//           </button>
//           {photo && (
//             <button onClick={() => setPhoto(null)}
//               style={{ marginLeft:6, fontSize:10, padding:'4px 8px', background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.2)', color:'var(--cat-a)', borderRadius:6, cursor:'pointer', fontFamily:'var(--font-body)', fontWeight:600 }}>
//               ✕ Remove
//             </button>
//           )}
//         </div>
...
//             style={{ flex: 1, minWidth: 70, fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 16 }} />
...
//         <button onClick={() => onSave({ category, basePrice, role, photo })}
...
//           <div style={{ fontFamily: 'var(--font-mono)', fontSize: 26, fontWeight: 600, color: 'var(--accent)' }}>{total}</div>
//         </div>
//         {['BAT','BWL','AR','WK'].map((r) => (
//           <div className="card" key={r} style={{ textAlign: 'center' }}>
//             <div className="stat-label">{roleLabels[r]}</div>
//             <div style={{ fontFamily: 'var(--font-mono)', fontSize: 22, fontWeight: 600, color: roleColors[r] }}>
...
//           <div style={{ fontFamily: 'var(--font-mono)', fontSize: 22, fontWeight: 600, color: 'var(--cat-c)' }}>{sentCnt}</div>
//         </div>
//       </div>
...
//             return (
//               <div key={p.id} style={{ background: 'var(--bg-card)', border: `1px solid ${needsSetup ? 'rgba(249,115,22,0.35)' : 'var(--border)'}`, borderLeft: `4px solid ${ri.color}`, borderRadius: 12, padding: '12px 14px', transition: 'all 0.2s' }}>

//                 {/* Main row */}
//                 <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'nowrap' }}>
//                   {/* Index */}
//                   <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-muted)', minWidth: 18, textAlign: 'right', flexShrink: 0 }}>{i + 1}</div>

//                   {/* Avatar */}
//                   <PlayerAvatar photo={p.photo} name={p.name} size={42} color={ri.color} />
...
//                       {p.addedToAuction && (
//                         <span style={{ fontSize: 10, background: 'rgba(34,197,94,0.15)', color: 'var(--cat-c)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 4, padding: '1px 6px', fontWeight: 700, whiteSpace: 'nowrap' }}>⚡ In Auction</span>
//                       )}
//                       {needsSetup && (
//                         <span style={{ fontSize: 10, background: 'rgba(249,115,22,0.15)', color: '#f97316', border: '1px solid rgba(249,115,22,0.3)', borderRadius: 4, padding: '1px 6px', fontWeight: 700, whiteSpace: 'nowrap' }}>⚠️ Needs Setup</span>
//                       )}
//                     </div>
//                     <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
//                       <span style={{ background: ri.bg, color: ri.color, border: `1px solid ${ri.border}`, padding: '2px 7px', borderRadius: 20, fontSize: 10, fontWeight: 700, whiteSpace: 'nowrap' }}>{ri.icon} {ri.label}</span>
//                       <span className={`category-chip ${p.category}`} style={{ fontSize: 10, padding: '1px 7px' }}>{p.category}</span>
//                       {p.phone && <span style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>📱 {p.phone}</span>}
//                     </div>
//                   </div>

//                   {/* Base price badge */}
//                   <div style={{ textAlign: 'center', minWidth: 50, flexShrink: 0 }}>
//                     <div style={{ fontSize: 9, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 600 }}>Base</div>
//                     <div style={{ fontFamily: 'var(--font-mono)', fontSize: 15, fontWeight: 700, color: 'var(--gold)' }}>
//                       {p.basePrice}<span style={{ fontSize: 9, color: 'var(--text-muted)' }}>pts</span>
//                     </div>
//                   </div>

//                   {/* Action buttons */}
//                   <div style={{ display: 'flex', gap: 5, flexShrink: 0 }}>
//                     <button onClick={() => !p.addedToAuction && onSendToAuction(p.id)} disabled={p.addedToAuction} title={p.addedToAuction ? 'Already in auction' : 'Send to Auction'}
//                       style={{ background: p.addedToAuction ? 'var(--bg-card2)' : 'rgba(34,197,94,0.12)', border: `1px solid ${p.addedToAuction ? 'var(--border)' : 'rgba(34,197,94,0.4)'}`, color: p.addedToAuction ? 'var(--text-muted)' : '#22c55e', padding: '7px 8px', borderRadius: 7, fontSize: 11, fontWeight: 700, cursor: p.addedToAuction ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-body)', whiteSpace: 'nowrap' }}>
//                       {p.addedToAuction ? '✓' : '⚡'}
//                     </button>
//                     <button onClick={() => setEditingId(isEditing ? null : p.id)} title="Edit player"
//                       style={{ background: isEditing ? 'var(--accent-glow)' : 'var(--bg-card2)', border: `1px solid ${isEditing ? 'var(--accent)' : 'var(--border)'}`, color: isEditing ? 'var(--accent)' : 'var(--text-muted)', padding: '7px 8px', borderRadius: 7, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
//                       ✏️
//                     </button>
//                     <button onClick={() => onRemove(p.id)} title="Remove player"
//                       style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', color: 'var(--cat-a)', padding: '7px 8px', borderRadius: 7, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
//                       🗑
//                     </button>
//                   </div>
//                 </div>
...
//                 {/* Inline assign panel */}
//                 {isEditing && (
//                   <AssignPanel player={p} onSave={(updates) => handleSaveEdit(p, updates)} onClose={() => setEditingId(null)} />
//                 )}
//               </div>
//             );
//           })}
